...
@section('page_title', 'Perbarui Prestasi')
@section('page_subtitle', 'Modifikasi catatan medali & capaian talenta kejuruan SMKN 1 CIOMAS')
...
<div class="space-y-6 max-w-4xl mx-auto font-sans">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-2 text-xs font-mono text-slate-500 uppercase tracking-wider">
            ...
            <span>/</span>
            <span class="text-purple-600 font-bold">Edit</span>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="student_name" id="student_name" value="{{ old('student_name', $achievement->student_name) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 focus:bg-white transition-all">
                ...
                <div>
                    ...
                    <input type="text" name="class" id="class" value="{{ old('class', $achievement->class) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 focus:bg-white transition-all">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $achievement->title) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 focus:bg-white transition-all">
            ...
            <div>
                ...
                <input type="date" name="achievement_date" id="achievement_date" value="{{ old('achievement_date', $achievement->achievement_date ? $achievement->achievement_date->format('Y-m-d') : '') }}" required class="w-full sm:w-1/2 px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 focus:bg-white transition-all">
            ...
            <div>
                <label class="block text-xs font-mono font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                    Foto Siswa / Dokumentasi Medali <span class="text-slate-400 lowercase font-normal">(kosongkan jika tidak ingin mengubah)</span>
                </label>

                @if($achievement->image)
                    <div class="mb-3 p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-4">
                        <img src="{{ $achievement->image_url }}" alt="{{ $achievement->student_name }}" class="w-16 h-16 object-cover rounded-lg border border-slate-200">
                        <div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" required class="w-full px-3.5 py-3 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 focus:bg-white transition-all leading-relaxed">{{ old('description', $achievement->description) }}</textarea>
