...
@section('page_title', 'Perbarui Pengumuman')
@section('page_subtitle', 'Modifikasi data rilis informasi resmi SMKN 1 CIOMAS')
...
<div class="space-y-6 max-w-4xl mx-auto font-sans">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('pengumuman.show', $announcement->slug) }}" target="_blank" class="px-3 py-1.5 text-xs font-mono font-semibold uppercase tracking-wider text-amber-600 hover:text-amber-700 bg-amber-50 border border-amber-200 rounded-lg transition-colors">
                Lihat di Web &rarr;
            </a>
    ...
    <form action="{{ route('admin.pengumuman.update', $announcement) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $announcement->title) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500 focus:bg-white transition-all">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="published_at" id="published_at" value="{{ old('published_at', $announcement->published_at ? $announcement->published_at->format('Y-m-d') : '') }}" required class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500 focus:bg-white transition-all">
                ...
                <div>
                    ...
                    <input type="date" name="expired_at" id="expired_at" value="{{ old('expired_at', $announcement->expired_at ? $announcement->expired_at->format('Y-m-d') : '') }}" class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500 focus:bg-white transition-all">
            ...
            <div>
                <label class="block text-xs font-mono font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                    Gambar / Banner Pengumuman <span class="text-slate-400 lowercase font-normal">(kosongkan jika tidak ingin mengubah)</span>
                </label>
                
                @if($announcement->image)
                    <div class="mb-3 p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-4">
                        <img src="{{ $announcement->image_url }}" alt="{{ $announcement->title }}" class="w-16 h-16 object-cover rounded-lg border border-slate-200">
                        <div>
                            <span class="text-xs font-mono font-bold text-slate-700 block uppercase tracking-wide">Gambar Saat Ini</span>
                            <span class="text-xs text-slate-500 block mt-0.5">Unggah gambar baru di bawah jika ingin menggantinya.</span>
                        </div>
                    </div>
                @endif
                ...
                <div id="announcement-preview-box" class="hidden mt-4 p-3 rounded-xl bg-slate-50 border border-slate-200">
                    <div class="text-xs font-mono font-semibold text-slate-500 mb-2">Pratinjau Gambar Baru:</div>
