...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('pengumuman.show', $announcement->slug) }}" target="_blank" class="px-3.5 py-2 text-sm font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 border border-blue-100 rounded-xl hover:bg-blue-100/70 transition-colors">
                Lihat di Web
            </a>
    ...
    <form action="{{ route('admin.pengumuman.update', $announcement) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-sm space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $announcement->title) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-slate-700">
                        <option value="published" {{ old('status', $announcement->status) === 'published' ? 'selected' : '' }}>Published (Langsung Tayang)</option>
                        <option value="draft" {{ old('status', $announcement->status) === 'draft' ? 'selected' : '' }}>Draft (Simpan Sementara)</option>
                        <option value="expired" {{ old('status', $announcement->status) === 'expired' ? 'selected' : '' }}>Expired (Arsip)</option>
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="published_at" id="published_at" value="{{ old('published_at', $announcement->published_at ? $announcement->published_at->format('Y-m-d') : '') }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
                ...
                <div>
                    ...
                    <input type="date" name="expired_at" id="expired_at" value="{{ old('expired_at', $announcement->expired_at ? $announcement->expired_at->format('Y-m-d') : '') }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-800 mb-1.5">
                    Gambar / Banner Pengumuman <span class="text-xs text-slate-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span>
                </label>
                
                @if($announcement->image)
                    <div class="mb-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center gap-4">
                        <img src="{{ $announcement->image_url }}" alt="{{ $announcement->title }}" class="w-20 h-20 object-cover rounded-xl border border-slate-200">
                        <div>
                            <span class="text-xs font-semibold text-slate-700 block">Gambar Saat Ini</span>
                            <span class="text-xs text-slate-400 block mt-0.5">Unggah gambar baru di bawah jika ingin menggantinya.</span>
                        </div>
                    </div>
                @endif
                ...
                <div id="announcement-preview-box" class="hidden mt-4 p-3 rounded-2xl bg-slate-50 border border-slate-200">
                    <div class="text-xs font-semibold text-slate-500 mb-2">Pratinjau Gambar Baru:</div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all leading-relaxed">{{ old('content', $announcement->content) }}</textarea>
