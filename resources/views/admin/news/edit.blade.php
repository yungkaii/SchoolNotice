    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">
                <a href="{{ route('admin.berita.index') }}" class="hover:text-blue-600 transition-colors">Berita Sekolah</a>
                <span>/</span>
                <span>Edit</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Berita Sekolah</h1>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('berita.show', $news->slug) }}" target="_blank" class="px-3.5 py-2 text-sm font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 border border-blue-100 rounded-xl hover:bg-blue-100/70 transition-colors">
                Lihat di Web
            </a>
            <a href="{{ route('admin.berita.index') }}" class="px-3.5 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                Kembali
            </a>
        </div>
    </div>
    ...
    <form action="{{ route('admin.berita.update', $news) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-sm space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $news->title) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="published_at" id="published_at" value="{{ old('published_at', $news->published_at ? $news->published_at->format('Y-m-d') : '') }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-800 mb-1.5">
                    Foto Utama / Thumbnail Artikel <span class="text-xs text-slate-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span>
                </label>
                
                @if($news->image)
                    <div class="mb-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center gap-4">
                        <img src="{{ $news->image_url }}" alt="{{ $news->title }}" class="w-20 h-20 object-cover rounded-xl border border-slate-200">
                        <div>
                ...
                <!-- Preview Box -->
                <div id="news-preview-box" class="hidden mt-4 p-3 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="12" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all leading-relaxed">{{ old('content', $news->content) }}</textarea>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.berita.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
                Batalkan
            </a>
